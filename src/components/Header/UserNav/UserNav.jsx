...
import { selectAvatarURL, selectName } from 'redux/User/user-selectors';
import { AccountStyled, ImgAccount, SvgAccount } from './UserNav.styled';
...
export default function UserNav({ setBurgerMenu, burgerMenu }) {
  ...
  return (
    <nav>
      ...
      >
        {avatar ? (
          <ImgAccount src={avatar} alt={name} />
        ) : (
          <SvgAccount
            viewBox="0 0 20 20"
            fill="none"
        ...
        {name.length > 7 ? `${name.slice(0, 4)}...` : name}
      </AccountStyled>
    </nav>
  );
}
